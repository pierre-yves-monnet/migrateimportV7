...
import org.bonitasoft.migrate.ImportProcessMgmt.ProcessInstanceDescription.TasksInstanceIndentified;
...
public class ImportProcessMgmt {
  ...
  public int importOneProcess(ImportV7 importV7, ProcessStatus processStatus, MonitorImport monitorImport, APISession apiSession) {
    ...
    try {
      ...
      if (pos == 1) {
        logger.info("No [--] in the file - can't decode the file name");
      ...
      for (EndEventDefinition endEventDefinition : listEndEvents) {
        ...
        if (endEventDefinition.getMessageEventTriggerDefinitions().size() == 0
            && endEventDefinition.getSignalEventTriggerDefinitions().size() == 0)
  ...
  public StatusImport importOneProcessInstance(Map<String, Object> record, ImportV7 importV7, ImportListener importListener) {
    ...
    try {
      ...
      searchOptionsBuilder.filter(ProcessInstanceSearchDescriptor.STRING_INDEX_1, processInstanceContext.caseIdV5);
      ...
      if (processInstanceContext.isActiveCase) {
        ...
        isAlreadyPresent = (searchProcessInstance.getCount() > 0);
        if (searchProcessInstance.getResult().size() > 0)
      ...
      processInstanceContext.variablesProcesses = decodeVariables((String) record.get(FileCsv.VARIABLES));
      processInstanceContext.variablesActivity = decodeVariables((String) record.get(FileCsv.VARIABLESACTIVITY));
      ...
      processInstanceContext.historyCase  = "STARTED: "+(startedWhen==null ? "": sdfHistory.format(startedWhen)) + "("+ startedBy+")";
      // who started it?
      ...
      if (listTaskIdentifier != null)
      {
        for (TaskDescription taskIdentifier : listTaskIdentifier) {
          if (taskIdentifier.contextInformation.equals("FINISHED"))
            processInstanceContext.historyCase  += ","+taskIdentifier.taskName + ": " + sdfHistory.format(taskIdentifier.dateExecution) + "(" + taskIdentifier.executedBy + ")";
        }
      }
      // -------------------- Time to skip now
      if (isAlreadyPresent) {
      ...
      if (operationUpdateIndex) {
        ...
        processAPI.updateProcessInstanceIndex(processInstanceContext.rootProcessInstance.getId(), Index.FIRST, (String) record.get(FileCsv.CASEID));
        monitorImport.endOperationMarker(tokenUpdateProcessInstanceIndex);

        // update the history
          if (importV7.historyInIndex) {
            String history = processInstanceContext.historyCase;
            if (history.length() > 255)
              history = history.substring(0, 250) + "...";

            processAPI.updateProcessInstanceIndex(processInstanceContext.rootProcessInstance.getId(), Index.SECOND, history);
        }
        ...
        playSql.updateSql("update ARCH_PROCESS_INSTANCE set STRINGINDEX1 = ? where SOURCEOBJECTID=?", listSqlParameters);
  ...
  public List<TaskDescription> decodeListTaskIdentifier(String sourceList) {
    ...
    for (String task : listTasks) {
      ...
      if (task.length() > 0) {
        ...
        listTaskIdentifier.add(decodeTaskIdentifier(task));
      }
    }
    return listTaskIdentifier;
  }

  public TaskDescription decodeTaskIdentifier(String sourceIdentifier) {
    ...
    try {
      ...
      taskIndentifier.error += "5 items minimum are expected :" + e.toString();
   ...
   */
  public VariablesDescription decodeVariables(String variablesJson) {
    ...
    if (variablesJson != null) {
      ...
      for (String key : variables.keySet()) {
        TaskDescription task = decodeTaskIdentifier(key);
  ...
  public static class ProcessInstanceDescription {
    ...
    private SearchResult<FlowNodeInstance> searchFlowNode;
    ...
    public Long getProcessInstanceFromProcessDefinition(TaskDescription taskIdentifierProcess) {
      ...
      for (String key : mapProcessInstanceByDefinition.keySet()) {
        if (isIdentical(key, processName)) {
     ...
     * @return
     */
    public TasksInstanceIndentified getTaskInstanceFromProcessDefinition(TaskDescription taskIdentifierProcess, ProcessAPI processAPI, MonitorImport monitorImport) {
      TasksInstanceIndentified tasksInstanceIndentified = new TasksInstanceIndentified();
